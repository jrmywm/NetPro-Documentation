...
2. Start PostgreSQL and the backend when dashboard/API integration is needed.
3. Start the frontend when the UI is needed.
4. Start `Policy-Server-VM`; verify its DPDK preparation service, then launch the Policy Server manually.
5. Start `NPB-VM`; prepare its three DPDK ports, then launch the NPB.
